...
class Database {
    ...
    */
    ...
    private $myQuery = "";
    // used for debugging process with SQL return
    private $numResults = "";
    // used for returning the number of rows
    ...
    public function connect() {
        if ( !$this->con ) {
            ...
            /*array_push($this->result, $this->myconn);*/
            // mysqli_connect() with variables defined at the start of Database class
            if ( $this->myconn->connect_error ) {
                array_push( $this->result, "Connect Error: ".$this->myconn->connect_error );
                return false;
                // Problem connecting to database return FALSE
            } else {
                $this->con = true;
                /*return true;*/
                return $this->myconn;
    ...
    public function escaper($esc_string) {
        
        return $this->myconn->real_escape_string($esc_string);
    ...
    public function sql( $sql ) {
        ...
        if ( $query ) {
            ...
            $this->numResults = $query->num_rows;//this function works up till line 87
            ...
            array_push( $this->result, $this->myconn->result->error() );
            return false;
    ...
    public function select( $table, $rows = '*', $join = null, $where = null, $order = null, $limit = null ) {
        ...
        if ( $this->tableExists( $table ) ) {
            ...
            if ( $query ) {
                ...
                // Loop through the query results by the number of rows returned
                for ( $i = 0; $i < $this->numResults; $i++ ) {
                    $r = $query->fetch_array(); //fetch array and internal counter++ so returns next result set = row
                    $key = array_keys( $r );//gets all array keys and returns as array
                    for ( $x = 0; $x < count( $key );
                    $x++ ) {
                        // Sanitizes keys so only alphavalues are allowed
                        if ( !is_int( $key[$x] ) ) {
    ...
    public function insert( $table, $params = array() ) {
        ...
        if ( $this->tableExists( $table ) ) {
            ...
            $this->myQuery = $sql;
            array_push($this->result,"Insert ID: ".$this->myconn->insert_id);
            // Pass back the SQL
    ...
    //Function to delete table or row( s ) from database fixed
    ...
    public function delete( $table, $where = null ) {
        ...
        if ( $this->tableExists( $table ) ) {
            ...
            if ( $del = $this->myconn->query( $delete ) ) {
                array_push( $this->result, "Affected Rows = ".$this->myconn->affected_rows );
                $this->myQuery = $delete;
                // Pass back the SQL
                return true;
                // The query exectued correctly
            } else {
                array_push( $this->result, " Error".$this->myconn->error );
                return false;
