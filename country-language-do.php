...
<?php
    ...
    if (isset($_GET['Code'])) {
      ...
      /*  $sql = "SELECT * FROM countrylanguage WHERE `CountryCode` = '" . $code . "'" ;
        ...
        $db = new Database();
        $db->connect();
        $db->select('countrylanguage','*','',"CountryCode = '$code'",'' ); //$table, $rows = '*', $join = null, $where = null, $order = null, $limit = null
        echo $db->getSql();
...
echo "<table class=\"table table-striped table-bordered table-hover\">
        <tr style=\"text-align: left\">
            <th>Country Code</th>
            <th>Language</th>
            <th>Percentage Spoken in Country</th>
...
for ($x=0;$x <  $amountr;$x++){
    echo "<tr>";    
        ...
        echo "<td>" . $res["$x"]['Language'] . "</td>";
        echo "<td>" . $res["$x"]['Percentage'] . "</td>";
        echo "<td>" . $res["$x"]['IsOfficial'] . "</td>";
    echo "</tr>";
}
?>
<!--END OF PAGE CODE-->
